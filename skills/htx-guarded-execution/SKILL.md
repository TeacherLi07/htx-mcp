...
After every submission, cancellation, or close request, reconcile the resulting order and position state. Refresh `trading/state.json` with the reconciled positions and open orders, and append the request, acknowledgement, and reconciled outcome to `trading/JOURNAL.md`. An acknowledgement is not a fill. Do not retry after a timeout or uncertain response until the order is reconciled by order ID or client order ID.
