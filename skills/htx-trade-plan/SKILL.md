...
Update `trading/TRADE_PLAN.md` with the resulting thesis, entry trigger, invalidation, target or exit logic, size rationale, and validation status; append `trading/JOURNAL.md` for the plan decision. Do not include API credentials, signed URLs, or unnecessary account identifiers. State plainly that validation and preview are not an order; a ready, authorized order should proceed to guarded execution rather than remain indefinitely in planning.
