...
When no immediate entry is justified, update `trading/TRADE_PLAN.md` with the watched instrument, exact trigger, invalidation, timeframe, and rationale; append `trading/JOURNAL.md` when that changes the plan. Then use `htx_wait_for_market_event` only with explicit price or indicator conditions. Pass `timeout_minutes` in whole minutes; its default is 60 and its maximum is 180 (up to three hours). The tool keeps a persistent public WebSocket per product, automatically responds to HTX application heartbeats, reconnects with bounded exponential backoff, and resubscribes all channels after a transient disconnect. Avoid short timeout cycles used merely to wake up and inspect the market. A timeout is a safety and plan-review boundary, not a market signal or a reason to manufacture a new observation. When a condition fires, refresh the snapshot and either advance the stated setup or retain the no-trade condition; do not create a new thesis merely because the wait returned.

To make the wait tool the only wake-up source, set the outer `yield_time_ms` to more than `timeout_minutes * 60 * 1000`, with response margin, and do not yield while the tool is pending. When the MCP host deadline is three hours, choose a wait below that ceiling to leave response margin. Never run multiple `htx_wait_for_market_event` calls in parallel: they do not independently wake the agent. Put every watched market/indicator condition in one call, set each condition's `product` and `instrument` when markets differ, and use `match="any"` when any one condition should wake the agent. After it returns, retrieve a fresh snapshot before making a new conclusion. Do not emit an order or call an execution tool while waiting.
